...
const PlaceOrder = () => {
    ...
    const handleIncrease = (id) => {
        ...
        const CountStatus = SelectedProduct[0]?.amount;
        let AmounyStatus = 0;

        if (CountStatus > 0) {
            let newAmount = parseInt(CountStatus) + 1;
            AmounyStatus = newAmount;
            setCount(AmounyStatus);
            console.log(AmounyStatus)
        }

        const amount = AmounyStatus;
        
        const ProductData = { amount }
        console.log(ProductData);
        axiosSecure.patch(`/addProducts/${id}`, ProductData)
                                    ...
                                    cart.map(item => <div className="flex mb-5" key={item._id}>
                                        ...
                                        <div className="lg:w-3/12 w-full justify-end md:px-0 px-10 flex lg:justify-center items-center">
                                            <div className="md:flex flex-row justify-center items-center">
                                                <button onClick={() => setDecrease(item._id)} className="px-3 py-2 text-lg bg-gray-200"><FaMinus></FaMinus></button>
                                                <p className="text-center px-2">{item.amount}</p>
